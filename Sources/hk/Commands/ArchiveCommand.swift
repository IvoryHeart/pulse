...
enum ArchiveCommand {
    ...
    private static func showStats(json: Bool) {
        do {
            ...
            statusLines.append("\(TerminalUI.colored("Location:", .boldWhite))    ~/.hk/health.db")
            ...
            if stats.snapshotCount > 500 {
                ...
                recLines.append("Run \(TerminalUI.colored("hk archive run", .cyan)) to compress and reclaim space.")
                ...
                recLines.append("Run \(TerminalUI.colored("hk archive run", .cyan)) to archive old data.")
                print(TerminalUI.box(width: 60, title: "Recommendation", sections: [recLines]))
                print()
            } else {
