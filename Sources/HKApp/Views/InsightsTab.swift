            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                if viewModel.trendPredictions.isEmpty {
                    emptyState(
                        ...
                        detail: "Run `hk log` regularly to build trend predictions"
                    )
                } else {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                if viewModel.appProfiles.isEmpty {
                    emptyState(
                        ...
                        detail: "Run `hk log` to start tracking app energy usage"
                    )
                } else {
                    // Header row
                    HStack {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                if viewModel.changelogEntries.isEmpty {
                    emptyState(
                        ...
                        detail: "Run `hk changelog scan` to start tracking"
                    )
                } else {
                    ...
                    if stats.fileSizeBytes > 50_000_000 {
                        HStack(spacing: 4) {
                            Image(systemName: "exclamationmark.triangle.fill")
                                .font(.system(size: 9))
                                .foregroundColor(.orange)
                            Text("Database is large. Run `hk archive run` to compress.")
                                .font(.system(size: 9))
                                .foregroundColor(.orange)
                        }
                    }
