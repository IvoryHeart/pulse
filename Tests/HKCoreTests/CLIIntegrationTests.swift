...
// Run `swift build` from the project root to produce `.build/debug/hk`.
...
struct CLIIntegrationTests {
    ...
    /// Path to the built `hk` binary, resolved relative to this source file.
    let binaryPath: String = {
        URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()   // HKCoreTests
            .deletingLastPathComponent()   // Tests
            .deletingLastPathComponent()   // project root
            .appendingPathComponent(".build/debug/hk")
            .path
    }()

    /// Run the `hk` binary with the given arguments and return stdout + exit code.
    ///
    ...
    @Test("hk --version prints version string and exits 0")
    func testVersion() throws {
        let result = try runHK(["--version"])
        #expect(result.exitCode == 0)
        #expect(result.stdout.contains("hk (housekeeping) v0.5.0"))
    }

    // MARK: - Help

    @Test("hk --help includes COMMANDS section and all major commands")
    ...
    @Test("hk status exits 0 and contains expected headers")
    func testStatus() throws {
        let result = try runHK(["status"])
        #expect(result.exitCode == 0)
        #expect(result.stdout.contains("HOUSEKEEPING"))
        #expect(result.stdout.contains("Health Score:"))
    }

    // MARK: - Score

    @Test("hk score exits 0 and contains expected headers")
    ...
    @Test("hk status --json exits 0 and produces valid JSON")
    ...
    @Test("hk status --json contains expected top-level keys")
    ...
    @Test("hk status --json healthScore.score is between 0 and 100")
    ...
    @Test("hk score --json exits 0 and produces valid JSON with expected keys")
    ...
    @Test("hk log --info exits 0 and contains database info")
    ...
    @Test("hk net speed exits 0 and contains throughput labels",
          .timeLimit(.minutes(1)))
    ...
    @Test("hk bt exits 0 and contains Bluetooth")
    ...
    @Test("hk bluetooth exits 0 and contains Bluetooth")
    ...
    @Test("hk bt --json exits 0 and produces valid JSON")
    ...
    @Test("hk net wifi exits 0 and contains WiFi-related content")
    ...
    @Test("hk net wifi --json exits 0 and produces valid JSON")
    ...
    @Test("hk net --json exits 0 and produces valid JSON with wifi field",
          .timeLimit(.minutes(1)))
    ...
    @Test("hk --help mentions bluetooth command")
    ...
    @Test("hk unknown_command outputs unknown command message")
