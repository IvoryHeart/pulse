...
struct ModelCodableTests {
    ...
    @Test("HKProcessInfo encode then decode preserves all fields")
    func processInfoRoundTrip() throws {
        let original = HKProcessInfo(
            pid: 1234, name: "/usr/sbin/httpd",
            cpuPercent: 45.7, memPercent: 2.3, rssBytes: 256_000_000
        )
        let data = try encoder.encode(original)
        let decoded = try decoder.decode(HKProcessInfo.self, from: data)
    ...
    func healthSnapshotRoundTrip() throws {
        ...
        let original = HealthSnapshot(
            ...
            topProcesses: [
                HKProcessInfo(pid: 1, name: "kernel_task", cpuPercent: 5.0,
